...
export class Cell<CellType extends number> {
  #position: Pos;
  #grid: Grid<CellType>;
	#index: number;
  type: CellType;
  ...
  constructor(type: CellType, x: number, y: number, dist: number, grid: Grid<CellType>) {
    this.#position = {
      x: x,
      y: y,
    }
...
export class Grid<CellType extends number> {
   ...
   constructor(rows: number, cols: number, def: (x: number, y: number) => CellType) {
    ...
    for (let y = 0; y < rows; y++) {
      ...
      for (let x = 0; x < cols; x++) {
				const cell = new Cell<CellType>(def(x, y), x, y, Infinity, this);
				cells[y * cols + x] = cell;
        grid[y][x] = cell;
      }
    }
  ...
  toString(t: number = Infinity) {
    let str = '';
		let cell;
    for (let row = 0; row < this.#rows; row++) {
      for (let col = 0; col < this.#cols; col++) {
				cell = this.#grid[row][col]
        str += (cell.type === -1 || cell.type >= t) ? '.' : '#'
      }
